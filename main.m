...
%% load data
jpm_sp500grow = load("data/JPMorgan U.S. Research Enhanced Equity Fund.txt");
jpm_smallcap = load("data/JPMorgan Small Cap Equity Fund.txt");
jpm_eqincome = load("data/JPMorgan Equity Income Fund.txt");

br_sp500grow = load("data/BlackRock SP-500 Growth ETF fund.txt");
br_smallcap = load("data/BlackRock Russell 2000 ETF fund.txt");
br_china = load("data/BlackRock MSCI China-ETF fund.txt");

FF3 = load("data/FF3.txt");
...
logrets = log(1 + asset);
cumexret = cumsum(logrets(:,1) - logrets);
cumexret = cumexret(:,2:3);

figure
subplot(2,1,1)
plot(matlabdate, value_br_lcap, "LineWidth",2.0)
legend('BlackRock SP-500 Growth ETF fund', 'Benchmark', 'Makret', 'Interpreter','latex',"Location","best")
title('Value of invested \$1','Interpreter','latex')
datetick

subplot(2,1,2)
plot(matlabdate, cumexret*100, "LineWidth", 2.0)
legend("over benchmark", "over market", "Location","best",'Interpreter','latex')
title("BlackRock SP-500 Growth ETF fund Log Excess Return, (\%) ", 'Interpreter','latex')
datetick
...
plot(matlabdate, cumexret*100, "LineWidth", 2.0)
legend("over benchmark", "over market", "Location","best",'Interpreter','latex')
title("JPMorgan Equity Income Fund, (\%) ", 'Interpreter','latex')
datetick
